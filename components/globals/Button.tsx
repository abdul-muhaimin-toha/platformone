...
export default function Button({
  ...
  const variants: Record<string, string> = {
    primary:
      'h-14 w-fit py-2 px-6 text-xl leading-normal font-semibold rounded-full text-white bg-pulse-pink-600 hover:bg-pulse-pink-800 duration-800',
    'primary-arrow':
      'h-14 w-fit py-2 px-6 text-xl leading-normal font-semibold rounded-full text-white bg-pulse-pink-600 hover:bg-pulse-pink-800 duration-800',
    secondary:
    ...
    back: 'group h-10 bg-transparent w-fit py-2 pl-2 pr-3 text-base leading-normal font-normal rounded-full text-[#000000] hover:text-white hover:bg-pulse-pink-700 duration-800 border border-border',
  };
  ...
  if (href) {
    return (
      <Link
        href={href}
        target={target} // ✅ use target if provided
