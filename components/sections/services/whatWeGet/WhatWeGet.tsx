...
const WhatWeGet: FC<WhatWeGetProps> = ({
  ...
  useGSAP(() => {
    ...
    if (svgRef.current && sectionRef.current) {
      ...
      const moveStrength = 18; // max movement in px
      const rotateStrength = 4; // max rotation in deg
      ...
      const handleMouseMove = (e: MouseEvent) => {
        ...
        const relX = (x / bounds.width - 0.5) * 2; // -1 to 1
      ...
      <div className="container-custom">
        <div className="w-full z-20 flex flex-col gap-16 py-32 items-center justify-center">
          <h2 className="text-white w-full md:gap-3 flex-row flex items-center text-[38px] md:text-[56px] md:leading-[1.28] font-bold gap-2 leading-[1.26]">
            ...
            <span
              ref={spanRef}
              className="h-2 w-[140px] md:w-[230px] md:h-3 inline-flex bg-linear-[90deg] rounded-[3px] from-pulse-pink-700/0 to-pulse-pink-700"
            />
          </h2>
          ...
          <h2
            data-aos="fade-up"
            className="text-white w-full text-[38px] lg:text-[56px] lg:leading-[1.28] font-bold gap-2 leading-[1.26]"
          >
            {closingText}{' '}
